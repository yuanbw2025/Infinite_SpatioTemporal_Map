...
| 依赖     | 当前 workspace 方向基本正确                      | domain/ports/application 物理分离 | 待 P1                    |
...
| 业务规则 | 部分图谱/时间线/研究规则位于大 adapter           | 用例与领域规则向内迁移            | 待 P1                    |
| 文件规模 | 7 个文件超过硬上限                               | 分阶段拆分且债务不增长            | 已锁基线                 |
| 测试     | 关键 Python 管线已有失败路径回归，UI 仍缺测试    | 测试矩阵与覆盖率门禁              | P0.5 完成，P0 阈值已锁定 |
| CI       | verify 已含契约、覆盖率、密钥与许可证门禁        | 继续加入 SBOM 与端到端检查        | P0.6 完成                |

锁定债务以 `config/architecture-debt-baseline.json` 为准；表格不是豁免来源。
