...
0.3 → 0.4 把版本/来源、影印页、地点名称和作品覆盖范围移入唯一所有者；0.4 → 0.5 把任意关系字符串迁移到版本化核心谓词；0.5 → 0.6 增加正式人工篇章对齐集合；0.6 → 0.7 增加地方社会高频谓词且不重写已有事实；0.7 → 0.8 增加空的来源关系集合且不猜测任何谱系。当旧历史名称或几何存在多个可能来源时，必须用 `--default-source-id` 明确策展归属；未知谓词必须在 JSON 对象形式的 `--predicate-map` 中逐项裁决。迁移器不会猜测，缺少权利说明、影印 URL、谓词映射或冗余字段冲突时会停止。
