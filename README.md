...
这是全新原创架构的首个可运行产品版本。方志书库、单层与并列阅读、IIIF Image API 瓦片深度缩放与版面联动、实体档案、证据回溯、知识图谱、日级历史时间线、人物行迹、地方社会专题、时空地图、文博专题、来源链、可解释研究规则、词面/语义/混合全库检索和数据状态页面均已接入统一应用服务。

代码架构、数据生产线、三类协作审核工作台、冲突安全的决策合并、不可变发布登记与自动质量门禁已经完成；仓库内的虚构样本可重复跑通“分段—文本分层—提取—审核—发布—原文回溯”。当前产品层面的主要缺口是接入来源与权利清晰的真实方志数据，并据真实资料逐项校定内容与交互细节。详见[完成度与剩余工作](docs/completion-status.md)。
...
- [0.8 字段字典](docs/field-dictionary.md)
...
- [来源链与研究规则规范](docs/source-provenance-and-research-rules.md)
- [IIIF 深度影像与语义检索规范](docs/iiif-and-semantic-search.md)
- [协作审核与发布管理规范](docs/collaboration-and-release-management.md)
